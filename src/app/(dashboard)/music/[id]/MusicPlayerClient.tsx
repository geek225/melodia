...
export default function MusicPlayerClient({ track }: { track: Track }) {
  ...
  const shareUrl = typeof window !== "undefined" ? `${window.location.origin}/share/${track.id}` : "";
      ...
      <div className="max-w-6xl mx-auto py-8">
        ...
        <div className="w-full lg:w-105 shrink-0 flex flex-col items-center space-y-10">
          ...
          <div className={`w-full aspect-square rounded-[40px] flex items-center justify-center relative overflow-hidden group transition-all duration-700 bg-white ${isPlaying ? 'scale-[1.02] shadow-[0_30px_60px_-15px_rgba(255,107,0,0.3)]' : 'shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)]'}`}>
            ...
            <div className="absolute inset-0 bg-white/40 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center z-20">
              <input 
                type="file" 
                ref={fileInputRef} 
                className="hidden" 
